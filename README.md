...
Список на доработку в конце:

 - Сейчас используется alert → позже заменить на визуальные уведомления (toast, snackbar).
 - Можно добавлять стили для успешного входа/регистрации и ошибок.
 - Пароли хранятся в localStorage → нормально для MVP, но в реальном проекте нужен хэш